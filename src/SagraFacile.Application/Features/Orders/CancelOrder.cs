...
public static class CancelOrder
{
        ...
        IOrderStateMachine stateMachine) : ICommandHandler<Command, CommandResult>
    {
        public async Task<CommandResult> Handle(Command command, CancellationToken ct)
        {
            ...
            var targetStatus = command.Actor.Role == OrderActor.Customer
                ? OrderStatus.CancelledByCustomer
                : OrderStatus.CancelledByOperator;

            try
            {
                order.TransitionTo(targetStatus, policy, command.Actor, command.Reason);
                await repository.SaveChangesAsync(ct);
                return new CommandResult(true, $"Order #{order.OrderNumber} cancelled");
            }
