...
public class GetReservationsHandlerTests
{
    ...
    public async Task Handle_ReturnsPagedResultsWithCorrectTotalCount()
    {
        ...
        _repository.GetPagedAsync(1, 1, 50, Arg.Any<ReservationStatusFilter>(), Arg.Any<CancellationToken>())
    ...
    {
        _repository.GetPagedAsync(1, 1, 50, ReservationStatusFilter.Called, Arg.Any<CancellationToken>())
            .Returns((new List<Reservation>(), 0));

        await _handler.Handle(new GetReservations.Query(1, Filter: ReservationStatusFilter.Called), CancellationToken.None);

        await _repository.Received(1).GetPagedAsync(1, 1, 50, ReservationStatusFilter.Called, Arg.Any<CancellationToken>());
    }

    [Fact]
    public async Task Handle_PassesPaginationToRepository()
    {
        _repository.GetPagedAsync(1, 2, 10, Arg.Any<ReservationStatusFilter>(), Arg.Any<CancellationToken>())
            .Returns((new List<Reservation>(), 0));

        await _handler.Handle(new GetReservations.Query(1, Page: 2, PageSize: 10), CancellationToken.None);

        await _repository.Received(1).GetPagedAsync(1, 2, 10, Arg.Any<ReservationStatusFilter>(), Arg.Any<CancellationToken>());
    }
